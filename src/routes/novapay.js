...
async function parseXml(str) {
  return xml2js.parseStringPromise(str, { explicitArray: false, ignoreAttrs: false });
}
...
async function authenticate(userId) {
  ...
  const body = `<tem:UserAuthenticationJWT>
    <tem:request>
      ...
      <tem:refresh_token>${refresh_token}</tem:refresh_token>
      <tem:login>${login}</tem:login>
      <tem:public_certificate>${public_certificate}</tem:public_certificate>
    </tem:request>
  ...
  const result = parsed?.['s:Envelope']?.['s:Body']?.['UserAuthenticationJWTResponse']?.['UserAuthenticationJWTResult'];
  if (!result || result.result !== 'ok') {
    throw new Error('NovaPay auth failed: ' + (result?.result || 'no response'));
  }

  // Зберігаємо нові токени (Refresh Token одноразовий!)
  await pool.query(
    `UPDATE integrations SET token=$1, public_certificate=$2, jwt_token=$3, jwt_expiry=$4, last_sync=NOW()
     WHERE user_id=$5 AND provider='novapay'`,
    [result.refresh_token, result.public_certificate, result.jwt,
     result.expiration ? new Date(result.expiration) : null, userId]
  );

  return result.jwt;
}

// Отримати JWT з кешу або авторизуватись заново
async function getJwt(userId) {
  ...
  if (jwt_token && jwt_expiry && new Date(jwt_expiry) > new Date(Date.now() + 60000)) {
    return jwt_token;
  }
  return authenticate(userId);
}

// --- Підключення: перший крок (зберегти credentials і отримати список підприємств) ---
router.post('/connect', auth, async (req, res) => {
  ...
  try {
    ...
    // Авторизуємось і отримуємо список підприємств
    const jwt = await authenticate(req.userId);
    const clientsXml = await soapRequest(`<tem:GetClientsList>
      <tem:request>
        <tem:request_ref></tem:request_ref>
        <tem:jwt>${jwt}</tem:jwt>
      </tem:request>
    ...
    const result = parsed?.['s:Envelope']?.['s:Body']?.['GetClientsListResponse']?.['GetClientsListResult'];
    if (!result || result.result !== 'ok') throw new Error('Помилка отримання підприємств: ' + result?.result);

    let clients = result.clients?.['Clients'] || [];
...
router.post('/accounts', auth, async (req, res) => {
  ...
  try {
    ...
    const parsed = await parseXml(xml);
    const result = parsed?.['s:Envelope']?.['s:Body']?.['GetAccountsListResponse']?.['GetAccountsListResult'];
    if (!result || result.result !== 'ok') throw new Error('Помилка отримання рахунків: ' + result?.result);

    let accounts = result.accounts?.['Accounts'] || [];
    if (!Array.isArray(accounts)) accounts = [accounts];

    res.json({ accounts: accounts.filter(a => a.statuscode === 'Active').map(a => ({ id: a.id, iban: a.IBAN, name: a.name, currency: a.currency })) });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// --- Зберегти вибраний рахунок і активувати інтеграцію ---
...
router.post('/sync', auth, async (req, res) => {
  try {
    ...
    if (!intRow.rows.length || !intRow.rows[0].enabled) {
      return res.status(400).json({ error: 'NovaPay не підключено або не активовано' });
    }
    ...
    const xml = await soapRequest(`<tem:GetPaymentsList>
      <tem:request>
        <tem:request_ref></tem:request_ref>
        <tem:jwt>${jwt}</tem:jwt>
    ...
    const parsed = await parseXml(xml);
    const result = parsed?.['s:Envelope']?.['s:Body']?.['GetPaymentsListResponse']?.['GetPaymentsListResult'];
    if (!result || result.result !== 'ok') {
      return res.status(400).json({ error: 'NovaPay GetPaymentsList: ' + (result?.result || 'no response') });
    }

    // Payments — CDATA-wrapped XML
    ...
    // Тільки проведені (StatusDocumentId 8=Conducted, 9=Advance)
      ...
      if (isIncome) {
        await pool.query(
          `INSERT INTO incomes (user_id, amount, type, source, description, date, transaction_time, bank_tx_id)
           VALUES ($1,$2,'Некласифіковано','NovaPay',$3,$4,$5,$6) ON CONFLICT (bank_tx_id) DO NOTHING`,
